...
const { isNull, map, isNumber, toNumber, isString, forEach } = require('lodash');
...
var nlshort = {
  ...
  async delay(second = 5, log = '', r = false) {  
    ...
    return new Promise(async (resolve, reject) => {
      log && console.log(log);
      console.log(obj.getRndInteger(max,min));
      await delay(obj.getRndInteger(max,min));
      resolve(true);
      return;
    });
  },
  ...
  async has_or_login(options){
    ...
    return new Promise(async function(resolve) {
      ...
      console.log('client.requested', client.credentials);
  ...
  async downloadImage(url, path) { 
    ...
    if( fs.existsSync(path) ) {
      console.log('Already available Image', path);
      return path;
    };
  ...
  async createStoryImage(post, output = ''){  
    ...
    // var serveruri = 'https://tranquil-fjord-55741.herokuapp.com'
    var nlpath = {
      feed: '',
      story: ''
    }
    ...
    await axios
      ...
      .then(async res => {
        ...
        await obj.delay(8)
        nlpath.story = await obj.downloadImage(`${serveruri}/uploads/${res.data.story}`, output('story'));
        nlpath.feed = await obj.downloadImage(`${serveruri}/uploads/${res.data.feed}`, output('feed'));
  ...
  createFilename(post = {}, last){
    return ( post.owner?.username || post.owner?.id || 'nill' ) + '_' + ( post.shortcode || post.id || post.__typename ) + '_' + last +  ( post.is_video ? '.mp4' : '.jpg' );
  },
  async downloadPostData(options = { post, path }) {
    ...
    filepath  = function(last){
      return './saved/' + obj.createFilename(post, last);
    };
  ...
  async sanitize_post( post ) {
    ...
    return new Promise(async function(resolve) {
      ...
      o.id                    = isString(post.id) && post.id.length <= 16 ? toNumber(post.id) : post.id;
      o.posttype              = post?.posttype;
      o.getfrom               = post?.getfrom || '';
      ...
      if( post.posttype === 'story' ) {
        o.tappable_objects    = post?.tappable_objects || [];
        o.taken_at_timestamp  = post?.taken_at_timestamp;
        o.expiring_at_timestamp= post?.expiring_at_timestamp;
      }
    ...
    return new Promise(async function(resolve) {
      ...
      if( users.length ) {
        for (let z = 0; z < users.length; z++) {
          var user = users[z];
          console.log('useruseruseruser', JSON.stringify(user), user);
